...
export default function AdminDashboard() {
  ...
  const saveRoom = async () => {
    ...
    try {
      ...
      if (isNewRoom) {
        const message = tempExtraItems.length > 0 
          ? `Room created successfully! ${tempExtraItems.length} extra item(s) added.`
            ...
            <div className="bg-white rounded-2xl shadow-md p-6 sm:p-8 space-y-6 border border-slate-100">
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm font-sans text-slate-500">User engagement tracking for personalized recommendations (views, clicks, interactions).</p>
                </div>
